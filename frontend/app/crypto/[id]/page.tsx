...
import { useCoin, useCoinHistory } from "@/hooks/useCrypto";
import { useWatchlist } from "@/hooks/useWatchlist";
import type { ChartRange } from "@/types/market";
import { formatCompactCurrency, formatCurrency, formatPercent, formatUpdatedAt } from "@/utils/format";

export default function CryptoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const [range, setRange] = useState<ChartRange>("1M");
  const { data, isLoading, isError, error, refetch } = useCoin(id);
  const history = useCoinHistory(id, range);
          ...
          <>
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="text-right">
                <p className="text-3xl font-semibold tabular-nums">{formatCurrency(data.price_aud)}</p>
                {data.change_24h_pct !== null && (
                  <p className={data.change_24h_pct >= 0 ? "text-gain dark:text-gain-dark" : "text-loss dark:text-loss-dark"}>
                    {formatPercent(data.change_24h_pct)} (24h)
                  </p>
                )}
              </div>
            </div>
            ...
            <PriceChart
              ...
              valueFormatter={(v) => formatCurrency(v, "AUD", 0)}
              positive={(data.change_24h_pct ?? 0) >= 0}
            />

            <dl className="grid grid-cols-2 gap-4 text-sm sm:grid-cols-4">
              <Stat label="Market Cap" value={data.market_cap_aud !== null ? formatCompactCurrency(data.market_cap_aud) : "—"} />
              <Stat label="24h Volume" value={data.volume_24h_aud !== null ? formatCompactCurrency(data.volume_24h_aud) : "—"} />
              <Stat label="24h Change" value={data.change_24h_pct !== null ? formatPercent(data.change_24h_pct) : "—"} />
